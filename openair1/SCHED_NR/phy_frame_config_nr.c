...
{
  DevAssert(tdd_config != NULL || period_cfg != NULL);
  if (tdd_config != NULL && period_cfg != NULL)
    AssertFatal(1 == 0, "Either tdd_config or period_cfg must be non NULL");
  ...
  int nb_slots_to_set = TDD_CONFIG_NB_FRAMES * nr_slots_per_frame[mu];
  int slot_index = 0;
  tdd_period_config_t *pc = tdd_config ? &tdd_config->period_cfg : period_cfg;
  tdd_bitmap_t *tdd_slot_bitmap = pc->tdd_slot_bitmap;
  int nrofDLSlotsToConfig = tdd_config ? 1 : pc->num_dl_slots;
  int nrofULSlotsToConfig = tdd_config ? 1 : pc->num_ul_slots;
  if (tdd_config) {
    tdd_config->tdd_numb_slots_frame = nr_slots_per_frame[mu];
    tdd_config->tdd_numb_period_frame = nb_periods_per_frame;
    tdd_config->tdd_numb_slots_period = nb_slots_per_period;
  }

  LOG_I(NR_PHY,
        "Set TDD Period Configuration: %d periods per frame, %d slots to be configured\n",
  ...
  while (slot_number != nb_slots_to_set) {
    slot_index = tdd_config ? slot_number % nb_slots_per_period : 0;
    ...
    if (period_cfg) {
      if ((nrofDLSymbolsInSlot + nrofULSymbolsInSlot) == 0)
        AssertFatal(nb_slots_per_period == (pc->num_dl_slots + pc->num_ul_slots),
                    "set_tdd_configuration_nr: given period is inconsistent with current tdd configuration, pc->num_dl_slots %d, "
                    "pc->num_ul_slots %d, nb_slots_per_period %d \n",
                    pc->num_dl_slots,
                    pc->num_ul_slots,
                    nb_slots_per_period);
      else {
        AssertFatal(nrofDLSymbolsInSlot + nrofULSymbolsInSlot < NR_NUMBER_OF_SYMBOLS_PER_SLOT,
                    ...
                    nrofDLSymbolsInSlot,
                    nrofULSymbolsInSlot);
        AssertFatal(nb_slots_per_period == (pc->num_dl_slots + pc->num_ul_slots + 1),
                    "set_tdd_configuration_nr: given period is inconsistent with current tdd configuration, pc->num_dl_slots %d, "
                    "pc->num_ul_slots %d, nrofMixed slots 1, nb_slots_per_period %d \n",
                    pc->num_dl_slots,
                    pc->num_ul_slots,
                    nb_slots_per_period);
      }
    ...
    if (tdd_slot_bitmap[slot_index].slot_type == TDD_NR_DOWNLINK_SLOT) {
      for (int number_of_symbol = 0; number_of_symbol < nrofDLSlotsToConfig * NR_NUMBER_OF_SYMBOLS_PER_SLOT; number_of_symbol++) {
        cfg->tdd_table.max_tdd_periodicity_list[slot_number]
            .max_num_of_symbol_per_slot_list[number_of_symbol % NR_NUMBER_OF_SYMBOLS_PER_SLOT]
            .slot_config.value = 0;
    ...
    if (tdd_slot_bitmap[slot_index].slot_type == TDD_NR_MIXED_SLOT) {
      ...
      // DL Symbols
      for (int number_of_symbol = 0; number_of_symbol < nrofDLSymbolsInSlot; number_of_symbol++) {
        list->max_num_of_symbol_per_slot_list[number_of_symbol].slot_config.value = 0;
      }
      // Flexible Symbols
      for (int number_of_symbol = nrofDLSymbolsInSlot; number_of_symbol < NR_NUMBER_OF_SYMBOLS_PER_SLOT - nrofULSymbolsInSlot;
           number_of_symbol++) {
        list->max_num_of_symbol_per_slot_list[number_of_symbol].slot_config.value = 2;
      }
      // UL Symbols
      for (int number_of_symbol = NR_NUMBER_OF_SYMBOLS_PER_SLOT - nrofULSymbolsInSlot;
           number_of_symbol < NR_NUMBER_OF_SYMBOLS_PER_SLOT;
           number_of_symbol++) {
        list->max_num_of_symbol_per_slot_list[number_of_symbol].slot_config.value = 1;
      }
      slot_number++;
    }
    // UPLINK SLOTS
    if (tdd_slot_bitmap[slot_index].slot_type == TDD_NR_UPLINK_SLOT) {
      for (int number_of_symbol = 0; number_of_symbol < nrofULSlotsToConfig * NR_NUMBER_OF_SYMBOLS_PER_SLOT; number_of_symbol++) {
        cfg->tdd_table.max_tdd_periodicity_list[slot_number]
            .max_num_of_symbol_per_slot_list[number_of_symbol % NR_NUMBER_OF_SYMBOLS_PER_SLOT]
            .slot_config.value = 1;
        if ((number_of_symbol + 1) % NR_NUMBER_OF_SYMBOLS_PER_SLOT == 0) {
  ...
  for (int s = 0; s < nb_slots_per_period; s++) {
