...
static void get_beam_config(NR_gNB_COMMON *common_vars)
{
  paramdef_t  MacRLC_Params[] = MACRLCPARAMS_DESC;
  paramlist_def_t MacRLC_ParamList = {CONFIG_STRING_MACRLC_LIST, NULL, 0};
  config_getlist(config_get_if(), &MacRLC_ParamList, MacRLC_Params, sizeofArray(MacRLC_Params), NULL);
  AssertFatal(MacRLC_ParamList.numelt < 2, "Not expecting more than 1 MAC RLC configuration\n");
  if (MacRLC_ParamList.numelt == 0) {
    common_vars->analog_bf = false;
    common_vars->num_beams_period = 1;
  } else {
    for (int i = 0; i < MacRLC_ParamList.numelt; i++) {
      if (i == 0) {
        common_vars->num_beams_period = *MacRLC_ParamList.paramarray[i][MACRLC_ANALOG_BEAMS_PERIOD_IDX].u8ptr;
        common_vars->analog_bf = *MacRLC_ParamList.paramarray[i][MACRLC_ANALOG_BEAMFORMING_IDX].u8ptr;
      }
      else
        AssertFatal(common_vars->analog_bf == *MacRLC_ParamList.paramarray[i][MACRLC_ANALOG_BEAMFORMING_IDX].u8ptr
                    && common_vars->num_beams_period == *MacRLC_ParamList.paramarray[i][MACRLC_ANALOG_BEAMS_PERIOD_IDX].u8ptr,
                    "Multiple L2 with different analog beamforming configurations are not supported\n");
    }
  }
}
