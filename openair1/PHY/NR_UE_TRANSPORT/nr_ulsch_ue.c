...
/* FIXME
 * Symbol mapping changed by rebase
 * PUSCH_PROC_STATS introduced by rebase
 */
                                 ...
                                 c16_t **txdataF)
{
  int nb_harq = 0;
  /* FIXME
   * The original version of UE encoding was mistakenly assuming multiple HARQ processes
   * Merge of branch NR_HARQ32_PROCESSES in develop revealed the mistake
   * The code below was hot fixed during a rebase on develop
   * A clean fix will be performed in a later commit after rebase completion
   */
  ...
  */
  // Hot-fix
  int harq_pid_int = phy_data->ulsch.pusch_pdu.pusch_data.harq_process_id;
  uint8_t harq_pid = harq_pid_int;
  if (UE->ul_harq_processes[harq_pid].ULstatus == ACTIVE)
    nb_harq++;
  ...
  for (uint8_t harq_pid = 0; harq_pid < NR_MAX_ULSCH_HARQ_PROCESSES; harq_pid++) {
    ...
    G[idx] = nr_get_G(nb_rb, number_of_symbols, nb_dmrs_re_per_rb, number_dmrs_symbols, unav_res, mod_order, Nl);
