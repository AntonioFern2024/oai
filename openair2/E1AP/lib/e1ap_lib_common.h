...
#define _E1_CHECK_WARNING(condition)                                                                          \
  do {                                                                                                        \
    if (condition)                                                                                            \
      PRINT_ERROR("E1AP decoding warning: %s:%d: '%s' not supported yet.\n", __FILE__, __LINE__, #condition); \
  } while (0)
...
#define _E1_EQ_CHECK_POINTER(A)                                                                     \
  do {                                                                                              \
    if (A == NULL)                                                                                  \
      PRINT_ERROR("E1AP decoding warning: %s:%d: %s shall not be null \n", __FILE__, __LINE__, #A); \
  } while (0)
...
#define _E1_EQ_CHECK_COUNT(A) _E1_EQ_CHECK_GENERIC(A > 0, "%d", A, 0);
