...
{
  E1AP_E1AP_PDU_t *pdu = calloc(1, sizeof(*pdu));
  ...
  ieC2->value.choice.SecurityInformation.securityAlgorithm.cipheringAlgorithm = msg->cipheringAlgorithm;
  OCTET_STRING_fromBuf(&ieC2->value.choice.SecurityInformation.uPSecuritykey.encryptionKey,
                       msg->encryptionKey,
                       E1AP_SECURITY_KEY_SIZE);

  asn1cCallocOne(ieC2->value.choice.SecurityInformation.securityAlgorithm.integrityProtectionAlgorithm,
                 msg->integrityProtectionAlgorithm);
  ...
  for (int i = 0; i < msg->numPDUSessions; i++) {
    ...
    ieC6_1->pDU_Session_ID = pdu_session->sessionId;
    ieC6_1->pDU_Session_Type = pdu_session->sessionType;

    INT8_TO_OCTET_STRING(pdu_session->nssai.sst, &ieC6_1->sNSSAI.sST);
    if (pdu_session->nssai.sd != 0xffffff) {
      ieC6_1->sNSSAI.sD = malloc(sizeof(*ieC6_1->sNSSAI.sD));
      AssertFatal(ieC6_1->sNSSAI.sD != NULL, "out of memory\n");
      INT24_TO_OCTET_STRING(pdu_session->nssai.sd, ieC6_1->sNSSAI.sD);
    }

    const security_indication_t *sec = &pdu_session->securityIndication;
    ieC6_1->securityIndication.integrityProtectionIndication = sec->integrityProtectionIndication;
    ieC6_1->securityIndication.confidentialityProtectionIndication = sec->confidentialityProtectionIndication;
    if (sec->integrityProtectionIndication != E1AP_ConfidentialityProtectionIndication_not_needed) {
      asn1cCalloc(ieC6_1->securityIndication.maximumIPdatarate, ipDataRate);
      ipDataRate->maxIPrate = sec->maxIPrate;
    }
    ...
    const UP_TL_information_t *UP_TL_information = &pdu_session->UP_TL_information;
    ...
    for (const DRB_nGRAN_to_setup_t *j = pdu_session->DRBnGRanList; j < pdu_session->DRBnGRanList + pdu_session->numDRB2Setup;
         j++) {
      ...
      case E1AP_ProtocolIE_ID_id_System_BearerContextSetupRequest:
        ...
        switch (System_BearerContextSetupRequest->present) {
          ...
          case E1AP_System_BearerContextSetupRequest_PR_e_UTRAN_BearerContextSetupRequest:
            _E1_CHECK_WARNING(System_BearerContextSetupRequest->present
          ...
          case E1AP_System_BearerContextSetupRequest_PR_nG_RAN_BearerContextSetupRequest:
            ...
            _E1_EQ_CHECK_INT(System_BearerContextSetupRequest->present,
                             E1AP_System_BearerContextSetupRequest_PR_nG_RAN_BearerContextSetupRequest);
            _E1_EQ_CHECK_POINTER(System_BearerContextSetupRequest->choice.nG_RAN_BearerContextSetupRequest);
            break;
          default:
        ...
        _E1_EQ_CHECK_POINTER(pdu2SetupList);
        ...
        for (int i = 0; i < pdu2SetupList->list.count; i++) {
          ...
          E1AP_SecurityIndication_t *securityIndication = &pdu2Setup->securityIndication;
          // Integrity Protection Indication (M)
          security_indication_t *sec = &pdu_session->securityIndication;
          ...
          // GTP Tunnel
          struct E1AP_GTPTunnel *gTPTunnel = pdu2Setup->nG_UL_UP_TNL_Information.choice.gTPTunnel;
          _E1_EQ_CHECK_POINTER(gTPTunnel);
          _E1_EQ_CHECK_INT(pdu2Setup->nG_UL_UP_TNL_Information.present, E1AP_UP_TNL_Information_PR_gTPTunnel);
          // Transport Layer Address
          UP_TL_information_t *UP_TL_information = &pdu_session->UP_TL_information;
          ...
          _E1_EQ_CHECK_COUNT(drb2SetupList->list.count);
          ...
          for (int j = 0; j < drb2SetupList->list.count; j++) {
            ...
            _E1_EQ_CHECK_COUNT(cellGroupList->list.count);
...
e1ap_bearer_setup_req_t cp_bearer_context_setup_request(const e1ap_bearer_setup_req_t *msg)
{
  ...
  cp.cipheringAlgorithm = msg->cipheringAlgorithm;
  cp.integrityProtectionAlgorithm = msg->integrityProtectionAlgorithm;
  ...
  cp.activityNotificationLevel = msg->activityNotificationLevel;
  cp.numPDUSessions = msg->numPDUSessions;
  // Copy PDU Sessions
  for (int i = 0; i < msg->numPDUSessions; i++) {
    cp.pduSession[i].sessionId = msg->pduSession[i].sessionId;
    cp.pduSession[i].sessionType = msg->pduSession[i].sessionType;
    cp.pduSession[i].nssai = msg->pduSession[i].nssai;
    cp.pduSession[i].securityIndication = msg->pduSession[i].securityIndication;
    cp.pduSession[i].UP_TL_information.tlAddress = msg->pduSession[i].UP_TL_information.tlAddress;
    cp.pduSession[i].UP_TL_information.teId = msg->pduSession[i].UP_TL_information.teId;
    cp.pduSession[i].numDRB2Setup = msg->pduSession[i].numDRB2Setup;
    // Copy DRB_nGRAN_to_setup_t array
    for (int j = 0; j < msg->pduSession[i].numDRB2Setup; j++) {
      cp.pduSession[i].DRBnGRanList[j].id = msg->pduSession[i].DRBnGRanList[j].id;
      cp.pduSession[i].DRBnGRanList[j].sdap_config = msg->pduSession[i].DRBnGRanList[j].sdap_config;
      cp.pduSession[i].DRBnGRanList[j].pdcp_config = msg->pduSession[i].DRBnGRanList[j].pdcp_config;
      cp.pduSession[i].DRBnGRanList[j].tlAddress = msg->pduSession[i].DRBnGRanList[j].tlAddress;
      cp.pduSession[i].DRBnGRanList[j].teId = msg->pduSession[i].DRBnGRanList[j].teId;
      cp.pduSession[i].DRBnGRanList[j].numDlUpParam = msg->pduSession[i].DRBnGRanList[j].numDlUpParam;
      // Copy up_params_t array
      for (int k = 0; k < msg->pduSession[i].DRBnGRanList[j].numDlUpParam; k++) {
        cp.pduSession[i].DRBnGRanList[j].DlUpParamList[k] = msg->pduSession[i].DRBnGRanList[j].DlUpParamList[k];
      }
      cp.pduSession[i].DRBnGRanList[j].numCellGroups = msg->pduSession[i].DRBnGRanList[j].numCellGroups;
      // Copy cell_group_t array
      for (int k = 0; k < msg->pduSession[i].DRBnGRanList[j].numCellGroups; k++) {
        cp.pduSession[i].DRBnGRanList[j].cellGroupList[k] = msg->pduSession[i].DRBnGRanList[j].cellGroupList[k];
      }
      cp.pduSession[i].DRBnGRanList[j].numQosFlow2Setup = msg->pduSession[i].DRBnGRanList[j].numQosFlow2Setup;
      // Copy qos_flow_to_setup_t array
      for (int k = 0; k < msg->pduSession[i].DRBnGRanList[j].numQosFlow2Setup; k++) {
        cp.pduSession[i].DRBnGRanList[j].qosFlows[k].qfi = msg->pduSession[i].DRBnGRanList[j].qosFlows[k].qfi;
        cp.pduSession[i].DRBnGRanList[j].qosFlows[k].qos_params = msg->pduSession[i].DRBnGRanList[j].qosFlows[k].qos_params;
      }
    }
    cp.pduSession[i].numDRB2Modify = msg->pduSession[i].numDRB2Modify;
    // Copy DRB_nGRAN_to_mod_t array
    for (int j = 0; j < msg->pduSession[i].numDRB2Modify; j++) {
      cp.pduSessionMod[i].DRBnGRanModList[j] = msg->pduSessionMod[i].DRBnGRanModList[j];
    }
  }
  return cp;
}
 ...
 */
bool eq_drb_to_setup(const DRB_nGRAN_to_setup_t *a, const DRB_nGRAN_to_setup_t *b)
{
 ...
 */
bool eq_drb_to_mod(const DRB_nGRAN_to_mod_t *a, const DRB_nGRAN_to_mod_t *b)
{
  // Similar to eq_drb_to_setup, with modifications if needed
  return eq_drb_to_setup((const DRB_nGRAN_to_setup_t *)a, (const DRB_nGRAN_to_setup_t *)b);
}

/**
 * @brief Equality check for PDU sessions to modify/setup
 */
bool eq_pdu_sessions(const pdu_session_to_setup_t *a, const pdu_session_to_setup_t *b)
{
...
bool eq_bearer_context_setup_request(const e1ap_bearer_setup_req_t *a, const e1ap_bearer_setup_req_t *b)
{
  ...
  for (int i = 0; i < a->numPDUSessions; i++)
    if (!eq_pdu_sessions(&a->pduSession[i], &b->pduSession[i]))
      return false;
  if (a->numPDUSessionsMod != b->numPDUSessionsMod)
    return false;
  for (int i = 0; i < a->numPDUSessionsMod; i++)
    if (!eq_pdu_sessions(&a->pduSessionMod[i], &b->pduSessionMod[i]))
      return false;
  return true;
}
