...
static int get_nb_pucch2_per_slot(const NR_ServingCellConfigCommon_t *scc, int bwp_size)
{
  ...
  int max_csi_reports = MAX_MOBILES_PER_GNB << 1; // 2 reports per UE
                             ...
                             const NR_UE_NR_Capability_t *uecap)
{
  ...
  config_pucch_resset0(scc, pucch_Config, uid, curr_bwp, uecap);
  config_pucch_resset1(scc, pucch_Config, uid, curr_bwp, uecap);
                                                   ...
                                                   const nr_mac_config_t *configuration)
{
  ...
  pucch_Config->resourceToAddModList = calloc(1, sizeof(*pucch_Config->resourceToAddModList));
  pucch_Config->resourceToReleaseList = NULL;
  config_pucch_resset0(scc, pucch_Config, uid, curr_bwp, NULL);
  config_pucch_resset1(scc, pucch_Config, uid, curr_bwp, NULL);
  set_pucch_power_config(pucch_Config, configuration->do_CSIRS);
