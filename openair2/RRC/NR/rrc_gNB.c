...
*/
static void rrc_gNB_process_RRCSetupComplete(gNB_RRC_UE_t *UE, NR_RRCSetupComplete_IEs_t *rrcSetupComplete)
//-----------------------------------------------------------------------------
{
  ...
  rrc_gNB_send_NGAP_NAS_FIRST_REQ(UE, rrcSetupComplete);
}
...
static void rrc_gNB_process_RRCReestablishmentComplete(gNB_RRC_UE_t *ue_p, const uint8_t xid)
{
  ...
  uint8_t new_xid = rrc_gNB_get_next_transaction_identifier(0);
...
static int handle_rrcReestablishmentComplete(gNB_RRC_UE_t *UE, const NR_RRCReestablishmentComplete_t *reestablishment_complete)
{
  ...
  rrc_gNB_process_RRCReestablishmentComplete(UE, reestablishment_complete->rrc_TransactionIdentifier);
...
static void handle_rrcSetupComplete(gNB_RRC_UE_t *UE, const NR_RRCSetupComplete_t *setup_complete)
{
  ...
  rrc_gNB_process_RRCSetupComplete(UE, setup_complete->criticalExtensions.choice.rrcSetupComplete);
  return;
}

static void handle_rrcReconfigurationComplete(gNB_RRC_UE_t *UE, const NR_RRCReconfigurationComplete_t *reconfig_complete)
{
  ...
  switch (UE->xids[xid]) {
    case RRC_PDUSESSION_RELEASE: {
      ...
      gtpv1u_delete_ngu_tunnel(0, &req);
    ...
    case RRC_PDUSESSION_ESTABLISH:
      if (UE->n_initial_pdu > 0) {
        ...
        rrc_gNB_send_NGAP_PDUSESSION_SETUP_RESP(UE, xid);
      break;
    case RRC_PDUSESSION_MODIFY:
      rrc_gNB_send_NGAP_PDUSESSION_MODIFY_RESP(UE, xid);
      break;
...
static int rrc_gNB_decode_dcch(const f1ap_ul_rrc_message_t *msg)
{
  gNB_RRC_INST *gnb_rrc_inst = RC.nrrrc[0];
  /* we look up by CU UE ID! Do NOT change back to RNTI! */
  rrc_gNB_ue_context_t *ue_context_p = rrc_gNB_get_ue_context(gnb_rrc_inst, msg->gNB_CU_ue_id);
  if (!ue_context_p) {
  ...
  if (ul_dcch_msg->message.present == NR_UL_DCCH_MessageType_PR_c1) {
    switch (ul_dcch_msg->message.choice.c1->present) {
      ...
      case NR_UL_DCCH_MessageType__c1_PR_rrcReconfigurationComplete:
        handle_rrcReconfigurationComplete(UE, ul_dcch_msg->message.choice.c1->choice.rrcReconfigurationComplete);
        break;

      case NR_UL_DCCH_MessageType__c1_PR_rrcSetupComplete:
        handle_rrcSetupComplete(UE, ul_dcch_msg->message.choice.c1->choice.rrcSetupComplete);
        break;
      ...
      case NR_UL_DCCH_MessageType__c1_PR_ulInformationTransfer:
        rrc_gNB_send_NGAP_UPLINK_NAS(UE, ul_dcch_msg);
        break;
      ...
      case NR_UL_DCCH_MessageType__c1_PR_rrcReestablishmentComplete:
        handle_rrcReestablishmentComplete(UE, ul_dcch_msg->message.choice.c1->choice.rrcReestablishmentComplete);
        break;
  ...
  while (1) {
    ...
    switch (ITTI_MSG_ID(msg_p)) {
      ...
      case F1AP_UL_RRC_MESSAGE:
        rrc_gNB_decode_dcch(&F1AP_UL_RRC_MESSAGE(msg_p));
...
//-----------------------------------------------------------------------------
void rrc_gNB_generate_SecurityModeCommand(gNB_RRC_UE_t *ue_p)
//-----------------------------------------------------------------------------
{
  uint8_t buffer[100];
