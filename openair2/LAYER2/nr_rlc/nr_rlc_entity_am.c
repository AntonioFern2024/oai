...
static int generate_tx_pdu(nr_rlc_entity_am_t *entity, char *buffer, int size)
{
  ...
  sdu = entity->tx_list;
  LATSEQ_P("D rlc.genpdu--rlc.seg", "::Rbuf%u.sn%u", sdu->sdu, sdu->sdu->sn);
                               ...
                               int sdu_id)
{
  ...
  LATSEQ_P("D rlc.buf--rlc.genpdu", "::PRbuf%u.Rbuf%u.sn%u.rlcpacketsize%u", buffer, sdu->sdu, sdu->sdu->sn, size);
