                  ...
                  int n_rb_sched[num_beams])
{
  ...
  while (iterator->UE != NULL) {
    ...
    * allocation after CCE alloc fail would be more complex) */
    ...
    sched_pdsch->rbStart = rbStart - bwp_info.bwpStart;
    sched_pdsch->tb_size = TBS;
    /* transmissions: directly allocate */
    n_rb_sched[beam.idx] -= sched_pdsch->rbSize;

    for (int rb = rbStart; rb < sched_pdsch->rbSize; rb++)
...
static void nr_dlsch_preprocessor(module_id_t module_id, frame_t frame, sub_frame_t slot)
{
  ...
  NR_ServingCellConfigCommon_t *scc = mac->common_channels[0].ServingCellConfigCommon;
  const int CC_id = 0;
  /* To compute the availabble resources in the slot
     we search for the smallest TDA in size to go
     through the VRB map */
  int max_start = 0;
  int min_len = NR_NUMBER_OF_SYMBOLS_PER_SLOT;
  const int tda = get_dl_tda(mac, scc, slot);
  UE_iterator(UE_info->list, UE) {
    NR_UE_sched_ctrl_t *sched_ctrl = &UE->UE_sched_ctrl;
    NR_UE_DL_BWP_t *current_BWP = &UE->current_DL_BWP;
    int startSymbolIndex, nrOfSymbols;
    const int coresetid = sched_ctrl->coreset->controlResourceSetId;
    const NR_PDSCH_TimeDomainResourceAllocationList_t *tdaList = get_dl_tdalist(current_BWP,
                                                                                coresetid,
                                                                                sched_ctrl->search_space->searchSpaceType->present,
                                                                                TYPE_C_RNTI_);
  ...
  for (int j = 0; j < num_beams; j++) {
    n_rb_sched[j] = 0;
    uint16_t *vrb_map = mac->common_channels[CC_id].vrb_map[j];
    for (int i = 0; i < bw; i++) {
      // if all the pdsch symbols are free
      if (!(vrb_map[i] & slbitmap)) {
        n_rb_sched[j]++;
      }
    }
  }
