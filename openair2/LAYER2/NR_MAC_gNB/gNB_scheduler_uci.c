...
static int get_pucch_index(int frame, int slot, const tdd_config_t *tdd_cfg, int sched_pucch_size)
{
                       ...
                       sub_frame_t slotP)
{
  ...
  UE_iterator(nrmac->UE_info.list, UE) {
    ...
    const int pucch_index = get_pucch_index(frameP, slotP, &nrmac->tdd_config, sched_ctrl->sched_pucch_size);
                           ...
                           sub_frame_t slot)
{
  ...
  UE_iterator(nrmac->UE_info.list, UE ) {
    ...
    for (int csi_report_id = 0; csi_report_id < csi_measconfig->csi_ReportConfigToAddModList->list.count; csi_report_id++){
      ...
      const int pucch_index = get_pucch_index(sched_frame, sched_slot, &nrmac->tdd_config, sched_ctrl->sched_pucch_size);
                          ...
                          int is_common)
{
  ...
  const int n_slots_frame = nr_slots_per_frame[ul_bwp->scs];
  const tdd_config_t *tdd_cfg = &mac->tdd_config;

  const bool tdd = tdd_cfg->is_tdd;
  ...
  for (int f = 0; f < fb_size; f++) {
    ...
    const int pucch_index = get_pucch_index(pucch_frame, pucch_slot, &mac->tdd_config, sched_ctrl->sched_pucch_size);
    NR_sched_pucch_t *curr_pucch = &sched_ctrl->sched_pucch[pucch_index];
    if (curr_pucch->active &&
...
void nr_sr_reporting(gNB_MAC_INST *nrmac, frame_t SFN, sub_frame_t slot)
{
  ...
  UE_iterator(nrmac->UE_info.list, UE) {
    ...
    for (int SR_resource_id = 0; SR_resource_id < pucch_Config->schedulingRequestResourceToAddModList->list.count;SR_resource_id++) {
      ...
      const int pucch_index = get_pucch_index(SFN, slot, &nrmac->tdd_config, sched_ctrl->sched_pucch_size);
      NR_sched_pucch_t *curr_pucch = &sched_ctrl->sched_pucch[pucch_index];
