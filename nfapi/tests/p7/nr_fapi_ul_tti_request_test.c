 ...
 */
/*! \file nfapi/tests/p7/nr_fapi_ul_tti_request_test.c
 * \brief
...
{
  beamforming_pdu->num_prgs = rand16_range(1, 275);
  beamforming_pdu->prg_size = rand16_range(1, 275);
  beamforming_pdu->dig_bf_interface = rand8();
  ...
  if (beamforming_pdu->num_prgs > 0 && beamforming_pdu->dig_bf_interface > 0) {
    ...
    if (!beamforming_pdu->prgs_list) {
      ...
      AssertFatal(1 == 0, "Error allocating prgs_list");
    }
    ...
    for (int prg_idx = 0; prg_idx < beamforming_pdu->num_prgs; prg_idx++) {
      ...
      if (!beamforming_pdu->prgs_list[prg_idx].dig_bf_interface_list) {
        ...
        AssertFatal(1 == 0, "Error allocating dig_bf_interface_list on idx %d", prg_idx);
      }
    }
  }
...
static void fill_ul_tti_request_srs_parameters(nfapi_v4_srs_parameters_t *params, uint8_t num_symbols){
  if(num_symbols > 0){
    params->symbol_list = calloc(num_symbols, sizeof(*params->symbol_list));
  }
  ...
  uint8_t nUsage = __builtin_popcount(params->usage);
...
static void fill_ul_tti_request_srs_pdu(nfapi_nr_srs_pdu_t *pdu)
{
  ...
  uint16_t t_srs_values[] = {1, 2, 3, 4, 5, 8, 10, 16, 20, 32, 40, 64, 80, 160, 320, 640, 1280, 2560};
  pdu->t_srs = t_srs_values[rand16_range(0, 17)];
  pdu->t_offset = rand16_range(0, 2559);
  fill_ul_tti_request_beamforming(&pdu->beamforming);
  fill_ul_tti_request_srs_parameters(&pdu->srs_parameters_v4, pdu->num_symbols);
}
