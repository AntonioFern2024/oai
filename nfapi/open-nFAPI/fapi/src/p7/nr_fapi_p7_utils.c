 ...
 */
/*! \file nfapi/open-nFAPI/fapi/src/p5/nr_fapi_p5_utils.c
 * \brief
...
void free_ul_tti_request(nfapi_nr_ul_tti_request_t *msg)
{
  for (int idx_pdu = 0; idx_pdu < msg->n_pdus; ++idx_pdu) {
    ...
    nfapi_nr_ul_beamforming_t *beamforming_pdu = NULL;
    switch (pdu->pdu_type) {
      case NFAPI_NR_UL_CONFIG_PRACH_PDU_TYPE:
        beamforming_pdu = &pdu->prach_pdu.beamforming;
        break;
      case NFAPI_NR_UL_CONFIG_PUSCH_PDU_TYPE:
        beamforming_pdu = &pdu->pusch_pdu.beamforming;
        free(pdu->pusch_pdu.pusch_ptrs.ptrs_ports_list);
        break;
      case NFAPI_NR_UL_CONFIG_PUCCH_PDU_TYPE:
        beamforming_pdu = &pdu->pucch_pdu.beamforming;
        break;
      case NFAPI_NR_UL_CONFIG_SRS_PDU_TYPE:
        beamforming_pdu = &pdu->srs_pdu.beamforming;
        if(pdu->srs_pdu.srs_parameters_v4.symbol_list){
          free(pdu->srs_pdu.srs_parameters_v4.symbol_list);
        }
        break;
      default:
        beamforming_pdu = NULL;
        break;
    }
    if (beamforming_pdu != NULL) {
      if (beamforming_pdu->num_prgs > 0) {
        for (int prg = 0; prg < beamforming_pdu->num_prgs; ++prg) {
          if (beamforming_pdu->dig_bf_interface > 0) {
            free(beamforming_pdu->prgs_list[prg].dig_bf_interface_list);
          }
        }
...
static void copy_ul_tti_beamforming(const nfapi_nr_ul_beamforming_t *src, nfapi_nr_ul_beamforming_t *dst)
{
  ...
  if (dst->num_prgs > 0) {
    dst->prgs_list = calloc(dst->num_prgs, sizeof(*dst->prgs_list));
  }

  for (int prg = 0; prg < dst->num_prgs; ++prg) {
    if (dst->dig_bf_interface > 0) {
      dst->prgs_list[prg].dig_bf_interface_list = calloc(dst->dig_bf_interface, sizeof(*dst->prgs_list[0].dig_bf_interface_list));
